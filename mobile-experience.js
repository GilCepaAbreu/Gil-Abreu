(() => {
  ...
  const enableMobileExperience = () => {
    if (!mobile.matches || document.documentElement.dataset.mobileExperienceReady) return;
    ...
    const contact = document.querySelector('#contacto, #contact');
  ...
  mobile.addEventListener?.('change', enableMobileExperience);
})();
